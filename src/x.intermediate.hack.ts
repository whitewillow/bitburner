...
export async function main(ns: NS): Promise<void> {
  const CHOICE: string = ns.args[0]?.toString() ?? 'normal';

  ns.disableLog('ALL');

  const waitTime = 2000;

  function executeScript(script: string, serverHost: string, available_threads: number, targetHost: string) {
  ...
  while (true) {
    ...
    const targets = getTargetNodesDetailed(ns, 'hackChance').filter(s => s.server.moneyAvailable ?? 0 > 5000);
    ...
    for (const bot of botServers) {
      ...
      for (const target of targets) {
        ...
        if (hasAdminRights && hasMoney && canHack(ns, target.id)) {
          ...
          const shouldWeaken = target.server.hackDifficulty ?? 0 > (target.server.requiredHackingSkill ?? 0) + 5;
          const shouldGrow = CHOICE !== 'maxcash' && (target.server.moneyAvailable ?? 0) < (target.server.moneyMax ?? 0 * 0.70);
